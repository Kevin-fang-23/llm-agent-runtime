...
class ToolRegistry:
    ...
    def get(self, name: str) -> ToolSpec:
        if name not in self._tools:
            raise ToolExecutionError(f"未知工具: {name}，可用工具: {sorted(self._tools)}")
    ...
    async def execute(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        """校验 → 执行 → 归一化输出。所有异常折叠为 ToolExecutionError。"""
        spec = self.get(name)
        ...
        try:
            ...
            raise ToolExecutionError(f"工具 {name} 执行超时（>{spec.timeout_s}s）")
        except ToolExecutionError:
            raise
        except Exception as e:  # noqa: BLE001 工具侧任何异常都折叠为可分类错误
            raise ToolExecutionError(f"工具 {name} 运行异常: {type(e).__name__}: {e}") from e
