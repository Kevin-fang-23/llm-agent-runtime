...
def test_l1_ip_rate_limit(client, monkeypatch):
    ...
    raw = client.raw
    # 夹具建租户已占 1 次 → 再放行 4 次后触发；被拒的是无效 key 请求也照计数（鉴权之前拦）
    codes = [raw.get("/api/tasks", headers={"X-API-Key": "bad"}).status_code for _ in range(8)]
    assert codes[0] == 401 and codes[3] == 401
    assert codes[4] == 429 and codes[-1] == 429
    r = raw.get("/api/tasks", headers={"X-API-Key": "bad"})
